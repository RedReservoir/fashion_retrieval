...
def compute_fine_grained_retrieval_average_precision(
    shop_img_embs,
    shop_item_ids,
    cons_img_embs,
    cons_item_ids,
    ...
    cons_avg_prec_arr = torch.empty((num_cons_imgs), dtype=float).to(device)
    ...
    if with_tqdm: cons_img_zidxs_chunk_gen = tqdm(
        cons_img_zidxs_chunk_gen,
        total=len(cons_img_zidxs_chunk_gen),
        miniters=int(np.ceil(len(cons_img_zidxs_chunk_gen)/40)),
        file=logger,
        ascii=False,
        desc="      Progress",
        ncols=0,
    ...
    for cons_img_zidxs_chunk in cons_img_zidxs_chunk_gen:
        ...
        num_chunk_cons_imgs = cons_img_zidxs_chunk.shape[0]
        ...
        shop_to_cons_hits = torch.eq(shop_to_cons_nearest_item_ids, cons_item_ids[cons_img_zidxs_chunk])
        ...
        cons_avg_prec_arr[cons_img_zidxs_chunk] = avg_prec_arr[:].double()

    # Composite metrics

    return cons_avg_prec_arr, cons_shop_item_id_counts
...
if __name__ == "__main__":
    ...
    log_filename = "eval_ctsrbm_accuracy_2__logs.txt"
    ...
    logger.print("Train split average precision begin")
    ...
    logger.print("  Computing retrieval average precision")
    ...
    cons_avg_prec_arr, cons_shop_item_id_counts =\
        compute_fine_grained_retrieval_average_precision(
            train_shop_img_embs,
            train_shop_item_ids,
            train_cons_img_embs,
            train_cons_item_ids,
    ...
    logger.print("  Computed retrieval average precision")
    ...
    eval_data["results"]["train"] = [
        ...
        in zip(train_cons_idxs, cons_avg_prec_arr, cons_shop_item_id_counts)
    ]

    logger.print("  Current memory usage:")
    logger.print(src.utils.nvgpu.sprint_memory_usage([eval_params["settings"]["device_idx"]], num_spaces=4))

    logger.print("Train split average precision end")
    ...
    logger.print("Validation split average precision begin")
    ...
    eval_data["results"]["val"] = [
        (img_idx.item(), retr_acc.item(), num_shop_imgs.item())
        for (img_idx, retr_acc, num_shop_imgs)
        in zip(val_cons_idxs, cons_avg_prec_arr, cons_shop_item_id_counts)
    ]

    logger.print("Current memory usage:")
    logger.print(src.utils.nvgpu.sprint_memory_usage([eval_params["settings"]["device_idx"]], num_spaces=2))

    logger.print("Validation split average precision end")
    ...
    logger.print("Test split average precision begin")
    ...
    eval_data["results"]["test"] = [
        (img_idx.item(), retr_acc.item(), num_shop_imgs.item())
        for (img_idx, retr_acc, num_shop_imgs)
        in zip(test_cons_idxs, cons_avg_prec_arr, cons_shop_item_id_counts)
    ]

    logger.print("Current memory usage:")
    logger.print(src.utils.nvgpu.sprint_memory_usage([eval_params["settings"]["device_idx"]], num_spaces=2))

    logger.print("Test split average precision end")
    ...
    eval_data_filename = os.path.join(
        experiment_dirname, "eval_ctsrbm_accuracy_2__data.json"
    )
