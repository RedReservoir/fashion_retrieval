...
def train_epoch(data_loader, with_tqdm=True):
    ...
    for batch in loader_gen:
        ...
        batch_loss, batch_len = batch_evaluation(batch)
        total_loss_item += batch_loss.item() * batch_len
...
if __name__ == "__main__":
    ...
    if finished:
        ...
        while not finished:
            ...
            start_time = time()
            train_loss = train_epoch(train_loader, with_tqdm = terminal_tqdm and not terminal_silent) 
            end_time = time()
            ...
            start_time = time()
            val_loss = eval_epoch(val_loader, with_tqdm = terminal_tqdm and not terminal_silent) 
            end_time = time()
            ...
            gpu_usage = utils.mem.list_gpu_usage(device_idxs)[0]["device_usage"]
            epoch_time = (train_epoch_time + val_epoch_time) * 100

            logger.print("Stage 1 GPU MEM B: {:d}".format(gpu_usage))
            logger.print("Stage 1 epoch time s: {:.3f}".format(epoch_time))
            ...
            experiment_data["settings"]["stage_1"]["learning_rate_list"].append(scheduler.get_last_lr())
    ...
    if finished:
        ...
        while not finished:
           ...
           ### Training & validation
            ...
            logger.print("Stage 2 GPU MEM B: {:d}".format(gpu_usage))
            logger.print("Stage 2 epoch time s: {:.3f}".format(epoch_time))
            ...
            experiment_data["settings"]["stage_2"]["learning_rate_list"].append(scheduler.get_last_lr())
    ...
    if finished:
        ...
        start_time = time()
        test_loss = eval_epoch(test_loader, with_tqdm = terminal_tqdm and not terminal_silent)
        end_time = time()
