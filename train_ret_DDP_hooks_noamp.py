...
def add_silu_activation_hook(module, activation_name, activation_dict):

    def my_hook(module, input, output):
        activation_dict[activation_name] = {
            "module_class": "SiLU",
            "input_nan": input[0].isnan().any().item(),
            "input_mean": input[0].mean().item(),
            "input_std": input[0].std().item(),
            "output_nan": output.isnan().any().item(),
...
def add_batchnorm2d_activation_hook(module, activation_name, activation_dict):

    def my_hook(module, input, output):
        activation_dict[activation_name] = {
            "module_class": "BatchNorm2d",
            "input_nan": input[0].isnan().any().item(),
            "input_mean": input[0].mean().item(),
            "input_std": input[0].std().item(),
            "output_nan": output.isnan().any().item(),
            "output_mean": output.mean().item(),
            "output_std": output.std().item(),
            "running_mean__mean": module.running_mean.detach().mean().item(),
            "running_mean__std": module.running_mean.detach().std().item(),
            "running_var__mean": module.running_var.detach().mean().item(),
            "running_var__std": module.running_var.detach().std().item()
        }

    module.register_forward_hook(my_hook)


def sprint_activation_subdict(activation_name, activation_subdict):

    if activation_subdict["module_class"] == "SiLU":
        return sprint_silu_activation_subdict(activation_name, activation_subdict)
    if activation_subdict["module_class"] == "BatchNorm2d":
        return sprint_batchnorm2d_activation_subdict(activation_name, activation_subdict)


def sprint_batchnorm2d_activation_subdict(activation_name, activation_subdict):

    # Input/Output

    if np.isnan(activation_subdict["input_mean"]):
        input_mean_str = "{:9s}".format("nan")
    else:
        input_mean_str = "{:+.2e}".format(activation_subdict["input_mean"])

    if np.isnan(activation_subdict["input_std"]):
        input_std_str = "{:9s}".format("nan")
    else:
        input_std_str = "{:+.2e}".format(activation_subdict["input_std"])

    if np.isnan(activation_subdict["output_mean"]):
        output_mean_str = "{:9s}".format("nan")
    else:
        output_mean_str = "{:+.2e}".format(activation_subdict["output_mean"])

    if np.isnan(activation_subdict["output_std"]):
        output_std_str = "{:9s}".format("nan")
    else:
        output_std_str = "{:+.2e}".format(activation_subdict["output_std"])

    # Running stats

    if np.isnan(activation_subdict["running_mean__mean"]):
        running_mean__mean_str = "{:9s}".format("nan")
    else:
        running_mean__mean_str = "{:+.2e}".format(activation_subdict["running_mean__mean"])

    if np.isnan(activation_subdict["running_mean__std"]):
        running_mean__std_str = "{:9s}".format("nan")
    else:
        running_mean__std_str = "{:+.2e}".format(activation_subdict["running_mean__std"])

    if np.isnan(activation_subdict["running_var__mean"]):
        running_var__mean_str = "{:9s}".format("nan")
    else:
        running_var__mean_str = "{:+.2e}".format(activation_subdict["running_var__mean"])

    if np.isnan(activation_subdict["running_var__std"]):
        running_var__std_str = "{:9s}".format("nan")
    else:
        running_var__std_str = "{:+.2e}".format(activation_subdict["running_var__std"])

    # sprint

    sprint_str = "{:30s} ({:15s}) | {:5s} {:10s} {:10s} -> {:5s} {:10s} {:10s}".format(
        activation_name,
        activation_subdict["module_class"],
        str(activation_subdict["input_nan"]),
        input_mean_str,
        input_std_str,
        str(activation_subdict["output_nan"]),
        output_mean_str,
        output_std_str
    )

    running_mean_str = "Running mean: " + running_mean__mean_str + " " + running_mean__std_str
    running_var_str = "Running var:  " + running_var__mean_str + " " + running_var__std_str

    return sprint_str + "\n" + running_mean_str + "\n" + running_var_str


def sprint_silu_activation_subdict(activation_name, activation_subdict):
...
def add_activation_hooks_effnetb3(backbone):

    activation_dict = {}

    ### features[0]

    module = backbone.features[0][1]
    activation_name = "features[0][1]"
    add_batchnorm2d_activation_hook(module, activation_name, activation_dict)

    module = backbone.features[0][2]
    activation_name = "features[0][2]"
    add_silu_activation_hook(module, activation_name, activation_dict)
    ...
    for idx_1 in range(2):
        ...
        for idx_2 in [0]:
            ...
            module = backbone.features[idx_0][idx_1].block[idx_2][1]
            activation_name = "features[{:d}][{:d}].block[{:d}][1]".format(idx_0, idx_1, idx_2)
            add_batchnorm2d_activation_hook(module, activation_name, activation_dict)

            module = backbone.features[idx_0][idx_1].block[idx_2][2]
            activation_name = "features[{:d}][{:d}].block[{:d}][2]".format(idx_0, idx_1, idx_2)
            add_silu_activation_hook(module, activation_name, activation_dict)
    ...
    module = backbone.features[8][1]
    activation_name = "features[8][1]"
    add_batchnorm2d_activation_hook(module, activation_name, activation_dict)

    module = backbone.features[8][2]
    activation_name = "features[8][2]"
    add_silu_activation_hook(module, activation_name, activation_dict)
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(loader_gen):
            
        batch_loss = batch_evaluation(batch, ret_model, device, logger, batch_idx, activation_dict)
        total_loss_item += batch_loss.sum().item()

        (batch_loss.mean() / max_acc_iter).backward()
        curr_acc_iter += 1
...
def batch_evaluation(
    ...
    logger.print("Batch {:5d}".format(batch_idx))

    # anc_emb

    anc_emb = ret_model(anc_imgs)

    if anc_emb.isnan().any().item():
        logger.print("WARNING: anc_emb has nan loss")
        for key, value in activation_dict.items():
            logger.print(sprint_activation_subdict(key, value))

    # pos_emb

    pos_emb = ret_model(pos_imgs)

    if pos_emb.isnan().any().item():
        logger.print("WARNING: pos_emb has nan loss")
        for key, value in activation_dict.items():
            logger.print(sprint_activation_subdict(key, value))
    ...
    if neg_emb.isnan().any().item():
        logger.print("WARNING: neg_emb has nan loss")
        for key, value in activation_dict.items():
            logger.print(sprint_activation_subdict(key, value))
...
def execute_stage_1_DDP(
    ...
    logger.print("  [Rank {:d}] Train / Val batches: {:5d} / {:5d}".format(rank, len(train_loader), len(val_loader)))
    ...
    logger.print("  [Rank {:d}] Layers Hooking...".format(rank))

    activation_dict = add_activation_hooks_effnetb3(backbone)

    logger.print("  [Rank {:d}] Layers Hooked".format(rank))
