...
if __name__ == "__main__":
    ...
    log_filename = "logs__preview.txt"
    ...
    #ctsrbm_dataset = deep_fashion.ConsToShopClothRetrBM(ctsrbm_dataset_dir, img_transform=backbone_image_transform, neg_aux_filename_id="test")
    ...
    stage_1_epoch_time = (train_epoch_time + val_epoch_time) / cutdown_ratio
    stage_1_total_time = stage_1_epoch_time * max_epochs

    ### Save last component checkpoint

    logger.print("  Saving last epoch model to \"last_stage_1_ckp__preview.pth\"")
    ...
    logger.print("  Saved last epoch model to \"last_stage_1_ckp__preview.pth\"")
    ...
    if finished:
        ...
        logger.print("  Loading last epoch model from previous stage from \"last_stage_1_ckp__preview\"")
        ...
        logger.print("  Loaded best model from previous stage from \"last_stage_1_ckp__preview\"")
    ...
    stage_2_epoch_time = (train_epoch_time + val_epoch_time) / cutdown_ratio
    stage_2_total_time = stage_2_epoch_time * max_epochs

    training_total_time = stage_1_total_time + stage_2_total_time

    logger.print(" -- PREVIEW --")
    logger.print(" Stage 1 Epoch Time:   ", utils.time.sprint_fancy_time_diff(stage_1_epoch_time))
    logger.print(" Stage 2 Epoch Time:   ", utils.time.sprint_fancy_time_diff(stage_2_epoch_time))
    logger.print(" Stage 1 Total Time:   ", utils.time.sprint_fancy_time_diff(stage_1_total_time))
    logger.print(" Stage 2 Total Time:   ", utils.time.sprint_fancy_time_diff(stage_2_total_time))
    logger.print(" Experiment Total Time:", utils.time.sprint_fancy_time_diff(training_total_time))
