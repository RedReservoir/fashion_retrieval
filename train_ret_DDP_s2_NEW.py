...
def execute_stage_2_DDP(
    ...
    experiment_name_s1 = exp_params.get("experiment_name_s1", None)
    if experiment_name_s1 is not None:
        experiment_dirname_s1 = os.path.join(pathlib.Path.home(), "data", "fashion_retrieval", experiment_name_s1)
    else:
        experiment_dirname_s1 = None

    log_filename = "train_ret_DDP_stage_2__logs.txt"
    ...
    if num_epochs == 0:
        ...
        if experiment_dirname_s1 is not None:

            best_stage_1_experiment_checkpoint_filename = os.path.join(
                experiment_dirname_s1, "train_ret_DDP_stage_1__best_ckp.pth"
            )
        ...
        last_stage_2_experiment_checkpoint_filename = os.path.join(
            experiment_dirname, "train_ret_DDP_stage_2__last_ckp.pth"
        )
    ...
    ctsrbm_dataset_options = exp_data["settings"]["stage_2"]["ctsrbm_dataset"]
    ...
    if rank == 0:    
        finished_mp.value = exp_data["results"]["stage_2"]["finished"] and not command_args.resume_experiment
    ...
    while not finished_mp.value:
        ...
        if num_epochs == 1 and exp_params["settings"]["stage_2"].get("eval_epoch_0", False):
            ...
            if rank == 0:
                ...
                exp_data["results"]["stage_2"]["epoch_0_train_mean_loss_list"] = list(train_mean_loss_list_mp)
                exp_data["results"]["stage_2"]["epoch_0_val_mean_loss_list"] = list(val_mean_loss_list_mp)
        ...
        if rank == 0:
            exp_data["results"]["stage_2"]["learning_rate_list"].append(scheduler.get_last_lr()[0])
            ...
            with src.utils.signal.DelayedInterrupt():
                ...
                if best_tracker.is_best(sum(train_mean_loss_list_mp)):
                    ...
                    best_stage_2_experiment_checkpoint_filename = os.path.join(
                        experiment_dirname, "train_ret_DDP_stage_2__best_ckp.pth"
                    )
                ...
                exp_data_filename = os.path.join(
                    experiment_dirname, "train_ret_DDP_stage_2__data.json"
                )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(exp_data_filename):
        ...
        exp_data["experiment_name_s1"] = experiment_name_s1
        exp_data["settings"] = {}
        exp_data["results"] = {}
    ...
    if src.utils.dict.chain_get(exp_data, "results", "stage_2") is None:
        ...
        exp_data["results"]["stage_2"]["learning_rate_list"] = []
    ...
    finished = exp_data["results"]["stage_2"]["finished"] and not command_args.resume_experiment
