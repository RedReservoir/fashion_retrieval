...
def execute_stage_1_DDP(
    ...
    ddp_order = exp_params["settings"].get("DDP_order", "new")
    ...
    ctsrbm_dataset = deep_fashion_ctsrbm.ConsToShopClothRetrBmkDataset(
        ...
        neg_img_filename_list_id=exp_params["settings"]["data_loading"]["neg_img_filename_list_id"]
    )
    ...
    if rank == 0:
        ...
        if train_random_seed is None:
            dataset_random_mode_mp.value = "none"
        elif type(train_random_seed) is str:
            if train_random_seed == "random":
                dataset_random_mode_mp.value = "random"
        elif type(train_random_seed) is int:
    ...
    elif dataset_random_mode_mp.value == "fixed":
        state = np.random.get_state()
        np.random.seed(dataset_random_seed_mp.value)
        np.random.shuffle(ctsrbm_train_idxs)
        np.random.set_state(state)
    elif dataset_random_mode_mp.value == "none":
        pass
...
if __name__ == "__main__":
    ...
    if not finished:
        ...
        dataset_random_mode_mp = manager.Value("s", "")
